...
BitcoinGUI::BitcoinGUI(QWidget *parent):
    ...
    setWindowTitle(tr("Nitrous") + " " + tr("Wallet"));
    qApp->setStyleSheet("QMainWindow { background:rgb(237, 241, 247); font-family:'Proxima Nova Rg'; } #toolbar2 { border:none;width:30px; background:rgb(8,0,61); }");
#ifndef Q_OS_MAC
    ...
    gifCamelUnlocked = new QMovie(":/movies/nitrous_unlocked", "gif", this);
    gifCamelLocked = new QMovie(":/movies/nitrous_locked", "gif", this);
    ...
    actionConvertCurrency->setToolTip("Click here to convert your Nitrous to USD and BTC.  Nitrous price is pegged to BTC/N2O market and BTC/USD market on Coinbase.");
...
void BitcoinGUI::createActions() {
   ...
   // tabGroup->addAction(messageAction);
    ...
    sendCoinsAction = new QAction(QIcon(":/icons/null"), tr("&Send N2O"), this);
    sendCoinsAction->setCheckable(true);
    sendCoinsAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_2));
    toolbarsend2->addAction(sendCoinsAction);
    tabGroup->addAction(sendCoinsAction);

    receiveCoinsAction = new QAction(QIcon(":/icons/null"), tr("&Receive N2O"), this);
...
void BitcoinGUI::setClientModel(ClientModel *clientModel) {
    ...
    if (clientModel) {
        ...
        if (clientModel->isTestNet()) {
            ...
            if (trayIcon) {
                trayIcon->setToolTip(tr("Nitrous client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon() {
    ...
    trayIcon->setToolTip(tr("Nitrous client"));
...
void BitcoinGUI::setNumConnections(int count) {
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Nitrous network", "", count));
}
...
void BitcoinGUI::dropEvent(QDropEvent *event) {
    if (event->mimeData()->hasUrls()) {
        ...
        if (nValidUrisFound) {
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid Nitrous address or malformed URI parameters."));
        }
    }
